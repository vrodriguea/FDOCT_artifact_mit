...
def create_dataset_solap(path, dataset_name='dataset_intensidad.npz', solap_initial=20, maxSolap_multiplier=0.6, porcentaje=0.6):
    """
    Esta función procesa archivos combinando sus partes reales e imaginarias
    para formar tomogramas. Luego, aplica un solapamiento para cada B-scan 
    del tomograma, segun los parametrsos ingresados, esto para uamnetar el conjunto de datos.
    ...
    - solap_initial (int, opcional): Valor inicial de solapamiento. Por defecto es 20.
    - maxSolap_multiplier (float, opcional): Multiplicador para calcular el solapamiento máximo a partir de la dimensión z. Por defecto es 0.6.
    - porcentaje (float, opcional): Porcentaje de incremento para el solapamiento en cada iteración. Por defecto es 0.6.
    ...
    savez_compressed(dataset_name, inputs_amplitud, targets_amplitud)
