...
def total_loss(y_true, y_pred, L=0.15, M=0.05, N=0.8):
...
def define_gan(g_model, d_model, image_shape, L=0.15, M=0.05, N=0.8):
...
# Función de entrenamiento modificada para múltiples combinaciones
def train_multiple(d_model, g_model, dataset, n_epochs=100, n_batch=16):
    results = []
    
    for L, M, N in combinations:
        print(f'Training with L={L}, M={M}, N={N}')
        gan_model = define_gan(g_model, d_model, image_shape, L=L, M=M, N=N)
        d_loss, g_loss = train(d_model, g_model, gan_model, dataset, n_epochs=n_epochs, n_batch=n_batch)
        
        # Almacena las pérdidas
        results.append({'L': L, 'M': M, 'N': N, 'd_loss': d_loss, 'g_loss': g_loss})
    
    # Guarda los resultados en un archivo
    np.savez('training_results.npz', results=results)

# Define las combinaciones de parámetros
L_values = [0.15, 0.2, 0.25]
M_values = [0.01, 0.05, 0.1]
N_values = [0.8, 0.7, 0.65]

# Genera todas las combinaciones de L, M y N
combinations = list(product(L_values, M_values, N_values))
